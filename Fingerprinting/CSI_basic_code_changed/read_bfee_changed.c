...
void read_bfee(unsigned char *inBytes, mxArray *outCell)
{
    ...
    /* it skipped byte 6 and 7*/
     unsigned int sq1 = inBytes[6]+inBytes[7]; /* CHANGED: extracting the packet sequence number from payload */
    ...
    unsigned long int NID=inBytes[20] + (inBytes[21] << 8) +
		(inBytes[22] << 16) + (inBytes[23] << 24);/* CHANGED: extracting the NIC ID from the beforming struct */
    ...
    mxDestroyArray(mxGetField(outCell, 0, "sq1"));/* CHANGED:*/
    ...
    mxDestroyArray(mxGetField(outCell, 0, "NID"));/* CHANGED:*/
    ...
    mxSetField(outCell, 0, "sq1", mxCreateDoubleScalar((double)sq1));/* CHANGED:*/
    ...
    mxSetField(outCell, 0, "NID", mxCreateDoubleScalar((double)NID));/* CHANGED:*/
	         ...
	         int nrhs, const mxArray *prhs[])
{
	...
	outCell = mxCreateStructMatrix(1, 1, 14, fieldnames);  /* CHANGED: to 14 feild recorded*/
