...
%
function ret = read_bf_file_changed(filename)
%% Input check
...
while cur < (len - 3)
    % Read size and code
    field_len = fread(f,1, 'uint16', 0, 'ieee-be');% this is telling you how many bytes of data
    code = fread(f,1); % one byte is code 
    cur = cur+3;
    
    if (code == 193) %hex2dec('c1') IWL_CONN_RX_MPDU     /* CHANGED:*/
        bytes = fread(f, field_len-1, 'uint8=>uint8');
        cur = cur + field_len - 1; % have read 31 bytes
...
%         cur = cur + field_len - 1;
    ...
    if (code == 187) %hex2dec('bb')) Beamforming matrix -- output a record
        count = count + 1;
        ret{count} = read_bfee_changed(bytes);
        ret{count}.NID=dec2hex(ret{count}.NID);  %CHANGED;
        %ret{count}.sq1=dec2hex(ret{count}.sq1);
        perm = ret{count}.perm;
        Nrx = ret{count}.Nrx;
